...
- **中英是页面级切换，不是逐句对照。** 早先两种语言逐句交错排，在摘要还是一句话时读着
  没问题；改成散文后一张卡片要 150~420 字，两种语言同屏就是一面上千字符的墙。现在读者
  在页头选语言，一次只看一种。
...
每篇产出 **一句话论点 + 3~5 段短段落**，长度由 `config.json` 的
`summaryMinChars` / `summaryMaxChars` / `summaryParaMaxChars` 控制
（当前 120~420，每段 90）。标准是**读完不用再打开原文**。
...
最后一条是补丁摞补丁踩出来的：只说「短句」和「宁可少讲一个点」，模型把它读成了「少写」，
实跑下来 62 分的文章只给 150 字。所以又加了一句**短句不等于少写**，明说有料的文章该写到
300 字以上，只是拆成 4~5 段；100 字上下只留给链接汇总和发布公告。实测（同一批文章）：

```
改前：313 字 / 3 段，单句最长 92 字
改后：308 字 / 4 段，单句最长 49 字，多数 15~35 字
```

句长 35 字是软约束，实测约 15% 的句子会超到 37~49 —— 那些多是「冒号 + 列举」或
「冒号 + 设问」，读着并不别扭，所以没有继续收紧。硬约束是每段 90 字，那条模型守得住。
