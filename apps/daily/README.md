...
目前 9 类：AI · 技术 · 商业 · 投资 · 经济 · 科学 · 设计 · 生活 · 人文。
每栏内的文章全部出卡片，没有数量上限。空分类不渲染，所以一天通常只出现 5~6 栏。
...
- **分类 tab 默认停在第一个分类，不是「全部」。** 不用「全部」是因为散文摘要太长、
  每篇又都是整张卡片，整页平铺读不完；「全部」仍在，留给需要一张图截完的场合。
  这里曾经要找「第一个真正**有卡片**的分类」，因为某天 AI 栏可能全是版本发布、分数集体
  低于卡片门槛，一打开就是一列光秃秃的链接。低分文章不再进页面之后，存在的栏必然有
  卡片，那次搜索已经没有东西可找，直接取 `groups[0]`（`DigestBody.tsx`）。

- **两个控件分居头条上下，这是有理由的。** 语言在头条**上面** —— 它连头条的正文一起改，
  控件放在它所改动的文字下方会读成互不相关。分类 tab 在头条**下面**，紧贴着它筛选的分栏
  —— 头条不属于任何分类，tab 放在它上面等于暗示一个对「你读到的第一样东西」不起作用的
  筛选器。
...
`publishMinScore`（默认 40）决定一篇文章进不进页面。低于它的**根本不写进 JSON**。
...
所以没有 thesis 的文章视为「未判定」，照常占位并以纯标题渲染，和门槛不存在时一样。
...
每篇产出 **一句话论点 + 4~7 段短段落**，长度由 `config.json` 的
`summaryMinChars` / `summaryMaxChars` / `summaryParaMaxChars` 控制
（当前 120~600，每段 90）。标准是**读完不用再打开原文**。
...
- 口语连接词（但是／不过／因为／所以／问题在于），明确禁用「具体来说」「值得注意的是」
  「综合权衡」「而非」「其中」「且」「基于」「使得」
...
`ZH_EXAMPLE` 也重写了，因为在 JSON mode 里**示例就是规范** —— 光改指令、留着旧腔调的
示例，模型跟示例走。
...
228）。因为 `ZH_EXAMPLE` 的段落只有 20~30 字，六段加起来才 180 —— 它和「400 字以上」
