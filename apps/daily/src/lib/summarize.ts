...
<!-- 这个小标题是**给模型看的字段名**，页面上印出来的已经是「为什么值得读」
     （见 lib/i18n 里的 whyItMatters）。这里故意没跟着改：prompt 的措辞是调过
     的，而下面整段本来就在定义要写什么，标题只是个路牌 —— 改它是一次生成行为
     上的改动，不是一次文案改动。注意这整段处在模板字符串里，不要在这里写反
     引号。 -->
